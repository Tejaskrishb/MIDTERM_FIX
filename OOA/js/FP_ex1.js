...
const divBy3or5 = anArray.filter(x => x % 3 === 0 || x % 5 === 0);
...
console.log(multipBy3); //[3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36] //FILTER AND MAP
...
const arraySum = anArray.reduce((tot, x) => tot + x, 0);
console.log(arraySum); //78

//6. The sum of all the numbers squared 
const arraySquaredSum = anArray.map(x => x**2).reduce((tot, x) => tot + x, 0);
